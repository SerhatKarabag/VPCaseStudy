...
namespace ThreadRace.Tests.EditMode
{
    internal static class RaceTestSupport
    {
        ...
        public const string SaveKey = "ThreadRace.Tests.Save.V3";
        public const int SaveVersion = RaceEventSettings.CurrentSaveSchemaVersion;
        public const int DefaultSeed = 123456;
        public static readonly DateTimeOffset DefaultStartUtc = DateTimeOffset.FromUnixTimeSeconds(1782864000L);
            ...
            bool rangedAiDelays = false)
        {
            if (rangedAiDelays)
            {
                return new RaceConfiguration(
                    ...
                    finishTarget,
                    CreateRewardTiers(rewardedPositions));
            }
        ...
        public static RewardTierDefinition[] CreateRewardTiers(int rewardedPositions)
        {
            if (rewardedPositions <= 0)
            {
                return Array.Empty<RewardTierDefinition>();
            }

            var tiers = new RewardTierDefinition[rewardedPositions];
            for (var i = 0; i < tiers.Length; i++)
            {
                var rank = i + 1;
                var amount = rank == 1 ? 1000 : rank == 2 ? 500 : 250;
                tiers[i] = new RewardTierDefinition(
                    rank,
                    "thread_race_rank_" + rank.ToString() + "_coins",
                    RewardType.Coins,
                    amount,
        ...
        public static RaceSaveData CaptureStartedSave(RaceEventSettings settings)
        {
            ...
            session.Start();
            return new RaceSaveDataMapper().Capture(settings, session, CreateStartedTiming(settings));
        }
        ...
        public static RaceSaveData CaptureRewardPhaseRewardedSave()
        {
            var settings = CreateSettings(finishTarget: 1);
            var session = CreateSession(settings);
            session.Start();
        ...
        public static RaceEventTimingState CreateStartedTiming(
            RaceEventSettings settings,
            DateTimeOffset? startUtc = null,
            DateTimeOffset? lastObservedUtc = null)
        {
            var start = startUtc.GetValueOrDefault(DefaultStartUtc);
            return RaceEventTimingState.Started(
                start,
                start.AddSeconds(settings.EventDurationSeconds),
                lastObservedUtc.GetValueOrDefault(start));
        }
        ...
        public static string SnapshotSignature(RaceSnapshot snapshot)
        {
            ...
            builder.Append(snapshot.RewardClaimed).Append('|');
        ...
        public static string SaveSignature(RaceSaveData saveData)
        {
            ...
            builder.Append(saveData.RewardClaimed).Append('|');
            builder.Append(saveData.Revision).Append('|');
            builder.Append(saveData.TimingData.HasStarted).Append(':');
            builder.Append(saveData.TimingData.StartUtcUnixSeconds).Append(':');
            builder.Append(saveData.TimingData.EndUtcUnixSeconds).Append(':');
            builder.Append(saveData.TimingData.LastObservedUtcUnixSeconds).Append('|');
            ...
            if (saveData.PlayerOutcome != null)
            {
                ...
                builder.Append(saveData.PlayerOutcome.IsRewardEligible).Append(':');
                builder.Append(saveData.PlayerOutcome.CompletionReason);
            }
