...
namespace ThreadRace.Gameplay.Application
{
    ...
    {
        public RaceSaveData Capture(RaceEventSettings settings, RaceSession session)
        {
            if (settings == null)
            {
            ...
            return session.CaptureSaveData(settings.SaveSchemaVersion);
        }
            ...
            RaceSaveRacerData playerData)
        {
            ...
            if (outcome.DidFinish)
            {
                ...
                var expectedReward = outcome.FinishPlacement.Value <= settings.RaceConfiguration.RewardedPositionCount;
        ...
        {
            if (phase != RacePhase.NotStarted && phase != RacePhase.Running && phase != RacePhase.Completed)
            {
